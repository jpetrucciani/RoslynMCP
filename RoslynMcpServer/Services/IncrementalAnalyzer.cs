...
namespace RoslynMcpServer.Services
{
    ...
    public class IncrementalAnalyzer
    {
        ...
        public async Task<AnalysisResult> AnalyzeIncrementallyAsync(
            Solution solution,
            IEnumerable<DocumentId>? changedDocuments = null
        )
        {
            var documentsToAnalyze =
                changedDocuments?.ToHashSet()
                ?? solution.Projects.SelectMany(p => p.Documents).Select(d => d.Id).ToHashSet();

            var result = new AnalysisResult { AnalysisStartTime = DateTime.UtcNow };
        ...
        private List<SymbolSearchResult> ExtractSymbols(
            SyntaxNode root,
            Document document,
            ...
            foreach (var node in root.DescendantNodes())
            {
                ...
                symbols.Add(
                    new SymbolSearchResult
                    {
                        ...
                        Location =
                            $"{document.Project.Name}:{Path.GetFileName(document.FilePath)}:{lineSpan.StartLinePosition.Line + 1}",
        ...
        private string GetSymbolSummary(ISymbol symbol)
        {
            ...
            {
                IMethodSymbol method =>
                    $"({string.Join(", ", method.Parameters.Select(p => $"{p.Type.Name} {p.Name}"))})",
        ...
        private void UpdateFileCache(
            ...
            List<ComplexityResult> complexityResults
        )
        {
