...
namespace RoslynMcpServer.Services
{
    public class SymbolSearchService
    {
            ...
            foreach (var type in symbolTypes.Split(',', StringSplitOptions.RemoveEmptyEntries))
            {
                switch (type.Trim().ToLower())
                {
                    ...
                    case "field":
                        ...
                        break;
                    case "event":
                        types.Add(SymbolKind.Event);
                        break;
                    case "namespace":
                        types.Add(SymbolKind.Namespace);
                        break;
                }
            }
        ...
        private IEnumerable<ISymbol> GetFilteredSymbols(
        ...
        private SymbolSearchResult CreateSearchResult(ISymbol symbol, Project project)
        {
            ...
            return new SymbolSearchResult
            {
                ...
                Location =
                    $"{project.Name}:{Path.GetFileName(location?.SourceTree?.FilePath)}:{lineNumber}",
        ...
        private async Task<IEnumerable<ISymbol>> FindSymbolsByNameAsync(
            Solution solution,
            string symbolName
        )
        {
        ...
        public async Task<SymbolInfo?> GetSymbolInfoAsync(string symbolName, string solutionPath)
        {
            ...
            if (location != null && location.SourceTree != null)
            {
                ...
                info.SourceLocation =
                    $"{Path.GetFileName(location.SourceTree.FilePath)}:{lineSpan.StartLinePosition.Line + 1}";
            }
