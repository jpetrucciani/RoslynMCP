...
namespace RoslynMcpServer.Tools
{
    ...
    public class CodeNavigationTools
    {
        ...
        public static async Task<string> SearchSymbols(
            ...
            [Description(
                "Symbol types to include: class,interface,method,property,field (comma-separated)"
            )]
                string symbolTypes = "class,interface,method,property",
        ...
        )
        {
            try
            {
                ...
                var results = await searchService.FindReferencesAsync(
                    symbolName,
                    solutionPath,
                    includeDefinition
                );
            ...
            catch (Exception ex)
            {
                ...
                logger?.LogError(
                    ex,
                    "Error finding references for symbol: {SymbolName}",
                    symbolName
                );
        ...
        )
        {
            try
            {
                ...
                var dependencies = await analysisService.AnalyzeDependenciesAsync(
                    solutionPath,
                    maxDepth
                );
        ...
        private static string FormatReferenceResults(IEnumerable<ReferenceResult> results)
        {
            ...
            {
                output.AppendLine(
                    $"**{Path.GetFileName(fileGroup.Key)}** ({fileGroup.Count()} references):"
                );
        ...
        private static string FormatDependencyAnalysis(DependencyAnalysis analysis)
        {
            ...
            if (analysis.NamespaceUsages.Any())
            {
                ...
                foreach (
                    var usage in analysis
                        .NamespaceUsages.OrderByDescending(n => n.UsageCount)
                        .Take(10)
                )
        ...
        private static string FormatComplexityResults(List<ComplexityResult> results)
        {
            ...
            {
                output.AppendLine(
                    $"**{result.ClassName}.{result.MethodName}** (Complexity: {result.Complexity})"
                );
            ...
            if (results.Count > 20)
                output.AppendLine(
                    $"... and {results.Count - 20} more methods with high complexity"
                );
        ...
        {
            var namespaceDeclaration = method
                .Ancestors()
